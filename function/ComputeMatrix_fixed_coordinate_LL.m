...
Num_Concavity = (Num_grid-1) * Num_grid^(d-1) * d;
...
for kk = 1:d
    ...
    for ii = 1:length(unique_rest_x)
        for jj = 1:length(unique_x(:,kk))-1
            ...
            Amat_afriat(row,length(x)*kk + ind_1) = -1;
            Amat_afriat(row,length(x)*kk + ind_2) = 1;
            row = row+1;
        end
    end
end
...
Amat = Amat_afriat;
bvec=zeros(length(Amat),1);

lb = [-inf(m,1);zeros(m*d,1)];
